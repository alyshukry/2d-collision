class Vector {
    ...
    addToBoth(x: number | Vector, y?: number): this {
        if (x instanceof Vector) {
            this.x += x.x
            this.y += x.y
    ...
    subtractFromBoth(x: number | Vector, y?: number): this {
        if (x instanceof Vector) {
            this.x -= x.x
            this.y -= x.y
...
}   export {Vector}; window.Vector = Vector
...
declare global {
    interface Window {
        ...
        createParticle: typeof createParticle
        createParticles: typeof createParticles
        editParticles: typeof editParticles
    }
}
...
class Particle{
    ...
    constructor(r: number, mass: number, element: HTMLElement | SVGElement, container: Container, id: number) {
    ...
    getCoordinates(element: HTMLElement | SVGElement): { x: number, y: number } {
    ...
    checkCollision(other: Particle): boolean { // Runs for each particle pair (unless a collision is detected, then it stops looking for other pairs)
        ...
        if (distance < minDist) { // Particles have collided
            ...
            if (this.vel.abs().magnitude() + other.vel.abs().magnitude() > 0.02) { // If particle has high enough velocity to reduce chaos
                ...
                if (distanceSquared === 0) return true // Avoid division by zero
    ...
    update(): void {
        ...
        (this.element as HTMLElement).style.transform = `translate(${this.pos.x - this.r}px, ${this.pos.y - this.r}px)`
...
function createParticle(radius: number, mass: number, element: HTMLElement | SVGElement, container: Container) {
    particleId += 1
...
function createParticles(radii: number, masses: number, elements: HTMLElement[] | SVGElement[], container: Container) {
...
interface ParticleEditOptions {
    ...
    addRadii?: number 
    addMasses?: number
    multiplyRadii?: number 
    multiplyMasses?: number
}
...
let holdClickInterval: number
let saveAcceleration: Vector = new Vector(0, 0)
let container: Container
// Initial mouse or touch down - start tracking
function startTracking(event: MouseEvent | TouchEvent): void {
